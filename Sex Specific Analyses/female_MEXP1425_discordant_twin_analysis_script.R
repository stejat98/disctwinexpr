...
## look at only female samples
Female_Sample_Info <- subset(Sample_Info, Sex_Group == 'female')

## separate Sample_Info (Female) table into two tables: one only for affected samples and one for the unaffected
healthy_samples <- subset(Female_Sample_Info, Healthy0.Obesity1 ==0)
diseased_samples <- subset(Female_Sample_Info, Healthy0.Obesity1 == 1)
...
#saveRDS(genelevelvalueMEXP1425df, '/home/st320/second_iter_gse_genelevel_dfs/female_genelevelvalue_plain_t_test_MEXP1425df.RDS')
