...
healthy_samples_female <- subset(Sample_Info, Healthy0.CFS1 ==0 & Sex_Group == 'female')
diseased_samples_female <- subset(Sample_Info, Healthy0.CFS1 == 1 & Sex_Group == 'female')

## affected group of samples
tone <- gsm_tables_GSE16059[,match(diseased_samples_female$GSM,colnames(gsm_tables_GSE16059))]
## unaffected group of samples (control)
ttwo <- gsm_tables_GSE16059[,match(healthy_samples_female$GSM,colnames(gsm_tables_GSE16059))]
...
genelevelvalgse16059_female <- lapply(testinputloop$Var1,getgenelevelidvalfunct)
## convert to data frame
genelevelvalgse16059df_female <- as.data.frame(genelevelvalgse16059_female)
## transpose data frame
genelevelvaluegse16059df_female <- t(genelevelvalgse16059df_female)
colnames(genelevelvaluegse16059df_female) <- c("meandiff", "stanerror","pval")
genelevelvaluegse16059df_female <- as.data.frame(genelevelvaluegse16059df_female)

## FDR-correction of p-values for each gene
FDR_GSE16059_female <- p.adjust(genelevelvaluegse16059df_female$pval, method="fdr")
## bind FDR values column to gene-level value results data frame
genelevelvaluegse16059df_female <- cbind(GENEID=testinputloop$Var1,genelevelvaluegse16059df_female,FDR_GSE16059_female )

## save data frame to your desired sub-directory
#saveRDS(genelevelvaluegse16059df_female, '/home/st320/second_iter_gse_genelevel_dfs/Female_genelevelvalue_plain_t_test_gse16059df.RDS')
