...
add_pheno <- MEXP1425proc@phenoData@data$Sample.Name ## used to find which two samples should use from triplets -- not using "Fat twin 9 slimmer"
...
# remove third twin in TWP 9 as it consist of a triplet
Sample_Info <- Sample_Info[-14,]

#saveRDS(Sample_Info, '/home/st320/MEXP_1425_sample_info.RDS')

## look at only male samples
Male_Sample_Info <- subset(Sample_Info, Sex_Group == 'male')

## separate Sample_Info (Male) table into two tables: one only for affected samples and one for the unaffected
healthy_samples <- subset(Male_Sample_Info, Healthy0.Obesity1 ==0)
diseased_samples <- subset(Male_Sample_Info, Healthy0.Obesity1 == 1)


diseased_samples <- diseased_samples[match(healthy_samples$TWP_NUM,diseased_samples$TWP_NUM),]

## male affected group of samples
tone <- gsm_tables_MEXP1425[,match(diseased_samples$Colnames_eset,colnames(gsm_tables_MEXP1425))]
## male unaffected group of samples (control)
...
## runs tloop() -- performs t-test --  over all the columns between both twin groups
...
## change column name to FDR
colnames(genelevelvalueMEXP1425df)[5] <- "FDR"

## save data frame to your desired sub-directory 
#saveRDS(genelevelvalueMEXP1425df, '/home/st320/second_iter_gse_genelevel_dfs/male_genelevelvalue_plain_t_test_MEXP1425df.RDS')