...
## subset data frame to get only FEMALE MZ pairs
MZ_twinsExpression_df <- subset(twinsExpression, TWIN > 0 & Sex == 2)
...
#mzdiscordatwinids <- c(2,3,18,147,157,167,191,201,202,266,283,285,289,298,303,307,314,315,332,359,361,366,377,385,423,457,480,501,532,597,642,643,675,713)
#inputmzdiscordaids <- mzdiscordatwinids[-c(6,24,32)]


#temp_df_MZ <- subset(twinsExpression, TWIN == 332)
...
## get corresponding probe-level ids
...
## reformat problematic probe_ids
...
#input_changing_id <- seconditergplwright_discordant_generaldf[,1][which(temp_check == FALSE)][10:30]

# <- is.element(seconditergplwright_discordant_generaldf[,1],intersect(probe_id_gsewright_discordant_general,seconditergplwright_discordant_generaldf[,1]))
...
## trim whitespace in gene id vector
...
## column bind FDR values column to gene-level value results data frame
...
## find # of FDR significant genes
sum(genelevelvaluegsewright_discordant_generaldf[,5] < 0.05)

## save data frame to your desired sub-directory 
#saveRDS(genelevelvaluegsewright_discordant_generaldf, '/home/st320/second_iter_gse_genelevel_dfs/female_genelevelvalue_plain_t_test_gsewright_discordant_generaldf.RDS')
