...
healthy_samples_male <- subset(Sample_Info, Healthy0.CFS1 ==0)
diseased_samples_male <- subset(Sample_Info, Healthy0.CFS1 == 1)
...
genelevelvalgse16059 <- lapply(testinputloop$Var1,getgenelevelidvalfunct)
## convert to data frame
genelevelvalgse16059df <- as.data.frame(genelevelvalgse16059)
## transpose data frame
genelevelvaluegse16059df <- t(genelevelvalgse16059df)
colnames(genelevelvaluegse16059df) <- c("meandiff", "stanerror","pval")
genelevelvaluegse16059df <- as.data.frame(genelevelvaluegse16059df)

## FDR-correction of p-values for each gene
FDR_GSE16059 <- p.adjust(genelevelvaluegse16059df$pval, method="fdr")

## bind FDR values column to gene-level value results data frame
genelevelvaluegse16059df <- cbind(GENEID=testinputloop$Var1,genelevelvaluegse16059df,FDR_GSE16059 )

## save data frame to your desired sub-directory 
#saveRDS(genelevelvaluegse16059df, '/home/st320/second_iter_gse_genelevel_dfs/genelevelvalue_plain_t_test_gse16059df.RDS')
