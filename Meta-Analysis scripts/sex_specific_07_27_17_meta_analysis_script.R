...
#dim(subset(male_genelevelvalacrossallstudiesdfmod, pval < 0.05 & NumPhenotypes > 1))
#dim(subset(female_genelevelvalacrossallstudiesdfmod, pval < 0.05 &NumPhenotypes > 1))
...
#sum(female_genelevelvalues_meta_generaldf_plus_symbols_updated$pval<0.05)
...
male_metasig_genes_df_FDR <- subset(male_genelevelvalues_meta_generaldf_plus_symbols_updated, FDR < 0.05 & NumPhenotypes > 1)
...
length(which(!is.na(male_temp_df_metasig_FDR$symbol))) ## 101 genes without na symbols
...
male_i2_genes_df <- subset(male_genelevelvalues_meta_generaldf_plus_symbols_updated, I2 ==0  & NumPhenotypes > 1)
male_ndx_i2_genes_df <- order(male_i2_genes_df$I2, decreasing = F)
male_temp_df_i2_genes_df <- male_i2_genes_df[male_ndx_metasig_FDR,]

i2_zero_sig_genes_male <- subset(male_temp_df_metasig_FDR, I2 == 0 & !is.na(symbol)) ## 97 
...
female_metasig_genes_df_FDR <- subset(female_genelevelvalues_meta_generaldf_plus_symbols_updated, FDR < 0.05 & NumPhenotypes > 1)
...
length(which(!is.na(female_temp_df_metasig_FDR$symbol)))  ## 102 genes without na symbols

nrow(subset(female_temp_df_metasig_FDR, I2 == 0))

i2_zero_sig_genes_female <- subset(female_temp_df_metasig_FDR, I2 == 0 & !is.na(symbol)) ## 99
