...
## get 95 percentile of absolute values of overall mean differences 
#quantile(abs(genelevelvaluegseall_studies_meta_generaldf_plus_symbols_updated$meandiff),0.95)


## subset to find FDR (of mean difference) significant genes 10/24/17
metasig_genes_df <- subset(genelevelvaluegseall_studies_meta_generaldf_plus_symbols_updated, FDR < 0.05 & NumPhenotypes > 1 & abs(meandiff) > quantile(abs(genelevelvaluegseall_studies_meta_generaldf_plus_symbols_updated$meandiff),0.95))
...
## read sig_genes_df
#sig_genes_df <- readRDS("/home/st320/sig_genes_df_10_22_17_RDS")


## check which (is any) meta-analytic sig genes appear significant in individual phenotypes as well
which(sapply(temp_df_metasig[1:200,]$symbol,function(x){length(which(sig_genes_df$symbol == x))}) > 1)
...
nrow(subset(temp_df_metasig, !is.na(symbol) & I2 == 0))
